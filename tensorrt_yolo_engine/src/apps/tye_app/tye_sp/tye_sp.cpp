...
static const int32_t     THIS_DFLT_ATTEN_DB       = -1;
static const double      THIS_DFLT_REF_LEVEL      = -20.0;
...
static sp_frontend     *this_p_frontend     = nullptr;
...
static void display_help( void )
{
    ...
    std::cout << "----------------------------------------------------------------------------------------------------";
    std::cout << std::endl << std::flush;
    std::cout << "USAGE: ./tye_sp [ARGS]" << std::endl << std::flush;
    std::cout << "----------------------------------------------------------------------------------------------------";
    ...
    std::cout << std::endl << std::flush;
    std::cout << "  --retune-port ....... Listen port for retune commands";
    std::cout << std::endl << std::flush;
    ...
    std::cout << std::endl << std::flush;
    std::cout << "  --sample-rate-mhz ... Radio stream sample rate, in MHz (see NOTES)";
    std::cout << std::endl << std::flush;
    std::cout << "  --center-freq-hz .... Radio stream center frequency, in Hz";
    std::cout << std::endl << std::flush;
    std::cout << "  --atten-db ..,,,..... Radio attenuation (see NOTES)";
    std::cout << std::endl << std::flush;
    std::cout << "  --ref-level ......... Radio stream reference level (receiver sensitivity)";
    std::cout << std::endl << std::flush;
    ...
    std::cout << std::endl << std::flush;
    std::cout << "  --boxes-plot ........ Display the boxes plot (images with detection boxes)";
    std::cout << std::endl << std::flush;
    std::cout << "  --history-plot ...... Display the history plot (history and image diff time-series)";
    std::cout << std::endl << std::flush;
    ...
    std::cout << "----------------------------------------------------------------------------------------------------";
    std::cout << std::endl << std::endl << std::flush;
    std::cout << "  [STREAM PROCESSING]" << std::endl << std::endl << std::flush;
    ...
    std::cout << "       => --sample-rate-mhz" << std::endl << std::flush;
    std::cout << "       => --center-freq-hz" << std::endl << std::flush;
    std::cout << "     OPTIONAL" << std::endl << std::flush;
    std::cout << "       => --ad-port [default is " << THIS_DFLT_AD_PORT << "]" << std::endl << std::flush;
    std::cout << "       => --cnc-port [default is " << THIS_DFLT_CNC_PORT << "]" << std::endl << std::flush;
    std::cout << "       => --retune-port [default is " << THIS_DFLT_RETUNE_PORT << "]" << std::endl << std::flush;
    ...
    std::cout << "       => --atten-db [default is " << THIS_DFLT_ATTEN_DB << "]" << std::endl << std::flush;
    std::cout << "       => --ref-level [default is " << THIS_DFLT_REF_LEVEL << "]" << std::endl << std::flush;
    ...
    std::cout << "       => --boxes-plot [default is false]" << std::endl << std::flush;
    std::cout << "       => --history-plot [default is false]" << std::endl << std::flush;
    std::cout << std::endl << std::flush;
    std::cout << "  [NOTES]" << std::endl << std::flush;
    std::cout << "     => --sample-rate-mhz [25, 50]" << std::endl << std::flush;
    std::cout << "     => --atten-db [-1 => AUTO] OR [0,5,10,15,20,25,30]" << std::endl << std::flush;
    std::cout << std::endl << std::flush;
    std::cout << "----------------------------------------------------------------------------------------------------";
    std::cout << std::endl << std::flush;
    ...
    std::cout << "----------------------------------------------------------------------------------------------------";
    std::cout << std::endl << std::endl << std::flush;
    std::cout << "  SUPPORTED RADIOS ARE" << std::endl << std::flush;
    std::cout << "    => Signal Hound SM200x, Signal Hound SM435x" << std::endl << std::flush;
...
static bool parse_cmdline_args( int32_t num_args, char *p_arg[] )
{
    struct option opt_list[] =
    {
        ...
        { "ad-port",         required_argument, nullptr,  3 },
        { "cnc-port",        required_argument, nullptr,  4 },
        { "retune-port",     required_argument, nullptr,  5 },
        { "database-creds",  required_argument, nullptr,  6 },
        { "database-ip",     required_argument, nullptr,  7 },
        { "database-port",   required_argument, nullptr,  8 },
        { "database-off",    no_argument,       nullptr,  9 },
        { "sample-rate-mhz", required_argument, nullptr, 'A'},
        { "center-freq-hz",  required_argument, nullptr, 'B'},
        { "atten-db",        required_argument, nullptr, 'C'},
        { "ref-level",       required_argument, nullptr, 'D'},
        { "power-offset",    required_argument, nullptr, 'E'},
        { "pixel-min-val",   required_argument, nullptr, 'F'},
        { "pixel-max-val",   required_argument, nullptr, 'G'},
        { "false-detect-w",  required_argument, nullptr, 'H'},
        { "false-detect-h",  required_argument, nullptr, 'I'},
        { "boxes-plot",      no_argument,       nullptr, 'J'},
        { "history-plot",    no_argument,       nullptr, 'K'},
        { "help",            no_argument,       nullptr, 'L'},
        {  0,                0,                 0,        0 }
    };
    ...
    uint16_t              retune_port     = THIS_DFLT_RETUNE_PORT;
    ...
    uint64_t              sample_rate_mhz = 0;
    uint64_t              center_freq_hz  = 0;
    int32_t               atten_db        = THIS_DFLT_ATTEN_DB;
    double                ref_level       = THIS_DFLT_REF_LEVEL;
    ...
    bool                  boxes_plot_on   = false;
    bool                  history_plot_on = false;
    int32_t               opt             = -1;
    int32_t               opt_idx         = -1;
    ...
    while ( true )
    {
        opt = getopt_long(num_args, p_arg, "0123456789ABCDEFGHIJKL", opt_list, &opt_idx);
        if ( opt == -1 ) { break; }
        ...
        switch ( opt )
        {
            ...
            case 5: // retune-port
                ...
                retune_port = (uint16_t)strtoul(optarg, nullptr, /*base*/10);
                if ( retune_port == 0 ) { goto DISPLAY_HELP; }
                break;

            case 6: // database-creds
            ...
            case 'A': // sample-rate-mhz

                sample_rate_mhz = (uint64_t)strtoull(optarg, nullptr, /*base*/10);
                if ( sample_rate_mhz == 0 ) { goto DISPLAY_HELP; }
                break;

            case 'B': // center-freq-hz

                center_freq_hz = (uint64_t)strtoull(optarg, nullptr, /*base*/10);
                if ( center_freq_hz == 0 ) { goto DISPLAY_HELP; }
                break;

            case 'C': // atten-db

                atten_db = (int32_t)strtol(optarg, nullptr, /*base*/10);
                break;

            case 'D': // ref-level

                ref_level = std::stod(std::string(optarg));
                break;
    ...
    {
        std::cout << std::endl << "ARG => --sample-rate-mhz REQUIRED WHEN --process-stream" << std::endl << std::flush;
        goto DISPLAY_HELP;
    }
    else
    {
        // verify that the sample rate is valid
        if ( ! ((sample_rate_mhz == 25) || (sample_rate_mhz == 50)) )
        {
            std::cout << std::endl << "ARG => --sample-rate-mhz [25, 50]" << std::endl << std::flush;
            goto DISPLAY_HELP;
        }
    }
    ...
    if ( center_freq_hz == 0 )
    {
        std::cout << std::endl << "ARG => --center-freq-hz REQUIRED WHEN --process-stream" << std::endl << std::flush;
        goto DISPLAY_HELP;
    }
    // validate center frequency ?? valid range is radio specific

    // verify that attenuation is valid
    if ( ! ((atten_db == -1 ) || (atten_db == 0 ) || (atten_db == 5 ) || (atten_db == 10) ||
            (atten_db ==  15) || (atten_db == 20) || (atten_db == 25) || (atten_db == 30)) )
    {
        std::cout << "ARG => --atten-db [-1 => AUTO] OR [0,5,10,15,20,25,30]" << std::endl << std::flush;
        goto DISPLAY_HELP;
    }
    ...
    this_p_config = new sp_config(gpus, engine_path, engines_per_gpu, ad_port, cnc_port, retune_port, database_creds,
                                  database_ip, database_port, database_off, THIS_FFT_SIZE, (sample_rate_mhz * 1000000),
                                  center_freq_hz, atten_db, ref_level, THIS_SCORE_THRESHOLD, THIS_NMS_THRESHOLD,
                                  power_offset, pixel_min_val, pixel_max_val, false_detect_w, false_detect_h,
                                  boxes_plot_on, history_plot_on);
...
static bool start_engine( void )
{
    ...
    std::cout << "   - [PORTS] [AD] " << this_p_config->ad_port() << " [CNC] " << this_p_config->cnc_port()
              << " [RETUNE] " << this_p_config->retune_port() << std::endl << std::flush;
...
static bool start_frontend( void )
{
    ...
    this_p_frontend = new sp_frontend(this_p_buffer_pool, this_p_config, this_p_engine);
    ...
    std::cout << "[STREAM PROCESSING MODE] [OK]" << std::endl << std::flush;
